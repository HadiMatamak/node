...
const stripe = require('stripe')('sk_test_51NIewABYsXuC75xWTAqQvvtyaRwJHWCjeG4y8fOgCzdR7hIeDn7W0FF1FZqdGFzfdCc60Ydrphsj6wujwGNDEifa005hhZzXfu');
...
app.post('/signin', (req, res) => {
  ...
  db.query(query, [email], (err, result) => {
    ...
    if (storedPassword === password) {
      // Fetch randomLetters for the user
      const fetchRandomLettersQuery = 'SELECT randomLetters FROM qr_codes WHERE userId = ?';
      db.query(fetchRandomLettersQuery, [userId], (err, qrResult) => {
        if (err) {
          console.error(err);
          res.status(500).send('Could not fetch random letters');
          return;
        }

        if (qrResult.length === 0) {
          res.status(500).send('Random letters data not found');
          return;
        }

        const { randomLetters } = qrResult[0];
        res.status(200).send({ message: 'Signin successful!', randomLetters });
      });
    } else {
